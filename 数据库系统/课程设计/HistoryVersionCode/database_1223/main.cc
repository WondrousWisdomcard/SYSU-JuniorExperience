...
{    
	...
	if (trees_->parallelBulkLoad(n_pts_, table, 8, 500)) return 1;
	gettimeofday(&end_t, NULL);
	...
	strcat(systemCmd, "./par_res ");
	strcat(systemCmd, "./ser_res");
	printf("%s\n", systemCmd);
	system(systemCmd);
	return 0;
}
