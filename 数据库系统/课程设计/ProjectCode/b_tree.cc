...
)
{
	...
	*/
	// BIndexNode *tempI = new BIndexNode();
	// BLeafNode *tempL = new BLeafNode();
	// int BIndexNodeCap = tempI->calcCapacity(this->file_->get_blocklength());
	// int BLeafNodeCap = tempL->calcCapacity(this->file_->get_blocklength());
	// delete tempI;
	// delete tempL;
	// tempI = NULL;
	// tempL = NULL;
	...
	while (lastEndBlock > lastStartBlock)
	{
		...
		for (int i = 0; i < threadNum; i++)
		{
			...
			BLIA[i].fileLock = &fileLock;										  // prevent multi-thread access file
		}
